...
var zGL_ShaderObject_Class = (function(gl, shaderLib){
	...
	function ShaderObject(codes={}, attributes={}, uniforms={}){
		...
		A_names.forEach( (name, index)=>{
			this.attributes[name] = new _Attribute(index, name, A_locations[name], attributes[name]);
			this.attributes.array.push( this.attributes[name] );
		} );

		this.uniforms = {};
		insert_prototype( this.uniforms, {array:[]} );
		U_names.forEach( (name, index)=>{
			this.uniforms[name] = new _Uniform(index, name, U_locations[name], uniforms[name]);
	...
	ShaderObject.prototype.externalStart = function(){
